# Code from https://github.com/outlines-dev/outlines/blob/main/outlines/caching.py
import asyncio
import functools
import hashlib
import os
...
def get_cache():
    ...
    The cache directory defaults to `HOMEDIR/.cache/function_sampler`, but this choice
    can be overridden by the user by setting the value of the `FUNCTION_SAMPLER_CACHE_DIR`
    ...
    if sampler_version != memory.get("__version__"):
        memory.clear()
    memory["__version__"] = sampler_version
...
def disable_cache():
    ...
    `function_sampler.cache.disable` should be called right after importing function_sampler:

    >>> import function_sampler.cache as cache
    >>> cache.disable()
