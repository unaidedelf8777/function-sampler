...
def tokenize_dicts(input_dicts, tokenizer,  exempt_keys=["required", "name", "type", "enum", "parameters"], root=True):
    """
...
def sample_top_p(scores: torch.FloatTensor, top_p: float, filter_value: float = -float("Inf"), min_tokens_to_keep: int = 1):
...
def sample_top_k(scores: torch.FloatTensor, top_k: float, filter_value: float = -float("Inf"), min_tokens_to_keep: int = 1):
...
def sample_repetition_penalty(input_ids: torch.LongTensor, scores: torch.FloatTensor, penalty: float) -> torch.FloatTensor:
...
def bundle_sampling(scores: torch.FloatTensor, input_ids: torch.LongTensor = None, temperature: float = None, top_p: float = None, top_k: int = None, repetition_penalty: float = None, filter_value: float = -float("Inf"), min_tokens_to_keep: int = 1) -> torch.FloatTensor:
